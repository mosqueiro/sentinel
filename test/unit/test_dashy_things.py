...
def test_valid_papel_address():
    from papellib import is_valid_papel_address

    main = valid_papel_address()
    test = valid_papel_address('testnet')

    assert is_valid_papel_address(main) is True
    assert is_valid_papel_address(main, 'mainnet') is True
    assert is_valid_papel_address(main, 'testnet') is False

    assert is_valid_papel_address(test) is False
    assert is_valid_papel_address(test, 'mainnet') is False
    assert is_valid_papel_address(test, 'testnet') is True


def test_invalid_papel_address():
    from papellib import is_valid_papel_address

    main = invalid_papel_address()
    test = invalid_papel_address('testnet')

    assert is_valid_papel_address(main) is False
    assert is_valid_papel_address(main, 'mainnet') is False
    assert is_valid_papel_address(main, 'testnet') is False

    assert is_valid_papel_address(test) is False
    assert is_valid_papel_address(test, 'mainnet') is False
    assert is_valid_papel_address(test, 'testnet') is False
...
def test_hash_function():
    ...
    hex_hash = "%x" % papellib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(papellib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(papellib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(papellib.blocks_to_seconds(16616)) == 2612035
